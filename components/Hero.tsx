                ...
                {status === 'success' ? (
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col gap-2 max-w-lg">
                        <div className={`flex h-12 md:h-14 border bg-white shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] hover:shadow-[6px_6px_0px_0px_rgba(234,88,12,1)] transition-shadow duration-300 rounded-sm overflow-hidden ${status === 'error' ? 'border-red-500' : 'border-slate-900'}`}>
                            ...
                            />
                            <button type="submit" disabled={status === 'loading'} className="bg-brand-600 hover:bg-brand-700 text-white font-black uppercase text-xs sm:text-sm md:text-lg px-4 sm:px-6 md:px-8 h-full transition-colors whitespace-nowrap flex items-center justify-center">
                                {status === 'loading' ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : 'Start Reading'}
                            </button>
                        </div>
                        {status === 'error' && (
